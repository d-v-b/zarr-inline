...
async def test_get_metadata_key_returns_json_bytes():
    store = ZarrInlineStore(MemoryBacking({"zarr.json": {"node_type": "group"}}))
...
async def test_get_byte_key_base64_decodes():
    store = ZarrInlineStore(MemoryBacking({"a/c/0": "AAECAwQFBgc="}))
...
async def test_read_only_store_rejects_set():
    store = ZarrInlineStore(MemoryBacking({}), read_only=True)
    with pytest.raises(ValueError):
        await store.set("a/c/0", buf(b"\x00"))


async def test_read_only_store_rejects_delete():
    store = ZarrInlineStore(MemoryBacking({"a/c/0": "AA=="}), read_only=True)
    with pytest.raises(ValueError):
        await store.delete("a/c/0")
...
async def test_strict_mode_rejects_invalid_document_on_construction():
    from zarr_inline.validator import Strictness, ValidationError

    # metadata key mapping to a non-object value violates R2
    with pytest.raises(ValidationError):
        ZarrInlineStore(MemoryBacking({"zarr.json": "not an object"}), strictness=Strictness.STRICT)
...
async def test_lenient_mode_warns_on_invalid_document_but_constructs():
    ...
    with _warnings.catch_warnings(record=True) as caught:
        ...
        store = ZarrInlineStore(MemoryBacking({"zarr.json": "not an object"}))
    # construction succeeded
    assert store is not None
    # at least one validation warning was surfaced
    assert any("zarr-inline validation" in str(w.message) for w in caught)
...
async def test_lenient_mode_no_warning_on_valid_document():
    ...
    with _warnings.catch_warnings(record=True) as caught:
        _warnings.simplefilter("always")
        ZarrInlineStore(MemoryBacking({"zarr.json": {"node_type": "group"}}))
    assert not any("zarr-inline validation" in str(w.message) for w in caught)
